...
class OfficeMonitor {
  ...
  async monitor() {
    ...
    this.logger.info(`launching monitor process at ${new Date().toISOString()} for ${this.office.name}`);
    ...
    while (!loginCaptchaSuccess) {
      ...
      try {
        ...
        if ((await this.page.content()).match(/account is blocked/)) {
    ...
    const citizenshipConfirmButtons = await this.page.$$('[value="Confirm"]');
    ...
    // fill out citizenship info (only req. for some this.offices e.g. SF)
    if (this.office.citizenship) {
      ...
      await this.page.keyboard.type(_.random(100000000, 999999999));
