...
const SLEEP_SUCCESS_PERIOD = 60000 * 60;
const CONSECUTIVE_ERROR_LIMIT = 5;
const CAPTCHA_ERROR_LIMIT = 5;
const MAX_ACCEPTABLE_DATE = moment(process.env.MAX_ACCEPTABLE_DATE, 'MM/DD/YYYY');

const getConfirmedScreenshotPath = (pid) => path.join(__dirname, '..', 'tmp', `${pid}_confirmed.jpeg`);
const getYellowDayPath = (pid) => path.join(__dirname, '..', 'tmp', `${pid}_yellow_day.jpeg`);
    ...
    while (!this.success) {
      try {
        ...
        if (this.account.success) {
          this.logger.info('appointment already booked on this account');
          this.page.waitFor(SLEEP_SUCCESS_PERIOD);
        }
        ...
        while (!this.success) {
          ...
          if (medElms.length) {
            this.logger.info('yellow elm detected');
            await this.page.screenshot({ path: getYellowDayPath(this.pid) });
          }
          ...
          if (openDayElms.length) {
            ...
            this.smtpClient.notifyMe(`SLOT FOUND IN ${this.office.name} ON ${dateStr}`);

            if (m.isBefore(MAX_ACCEPTABLE_DATE)) {
              await Promise.all([
                openDayElm.click(),
                this.page.waitForNavigation({ waitUntil: 'networkidle2' }),
              ]);
              ...
              this.smtpClient.notifyMe(
                `APPOINTMENT BOOKED IN ${this.office.name} ON ${dateStr}`,
                `user: ${this.account.email}, pw: ${this.account.password}`,
              );
            ...
            await this.page.waitForSelector('#ctl00_ContentPlaceHolder1_lnkBack');
            ...
            await this.page.waitFor(getRandomDelay(REFRESH_PERIOD));
            await this.page.waitForSelector('#ctl00_ContentPlaceHolder1_acc_datiAddizionali1_btnContinua');

            // Session expired
            if ((await this.page.$('#ctl00_ContentPlaceHolder1_acc_datiAddizionali1_btnContinua')) == null) {
              break;
            }
