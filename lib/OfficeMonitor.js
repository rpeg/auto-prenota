...
const {
  getLoginPage, getRandomDelay, getRandomProfession, chromeOptions,
} = require('./utils');
...
const MAX_ACCEPTABLE_DATE = moment(process.env.MAX_ACCEPTABLE_DATE, 'DD/MM/YYYY');
...
class OfficeMonitor {
  ...
  async launch() {
    try {
      const forceAccountCreate = this.account && this.account.blocked;

      this.account = await this.accountManager.create(forceAccountCreate);
    ...
    setTimeout(async () => { await this.launch(); }, period);
  }
}
