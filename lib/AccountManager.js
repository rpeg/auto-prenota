...
class AccountManager {
  ...
  saveAccount() {
    fs.writeFileSync('../tmp/account', JSON.stringify(this.account));
  }

  deleteSavedAccount() {
    fs.writeFileSync('../tmp/account', '');
  }

  async getAccount() {
    try {
      fs.readFile('../tmp/account', (err, data) => {
      ...
      if (_.isEmpty(this.account) || !this.account.activated) {
